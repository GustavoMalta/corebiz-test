...
function App() {
  ...
  function monetize(value: string | number) {
    value = String(value);
    return `${value.slice(0, value.length - 2)},${value.substring(
      value.length - 2,
    )}`;
  }
                  ...
                  <div className="card-content">
                    ...
                    <div className="card-content-rate">
                      <Rating
                        readOnly
                        icon={
                          <FiStar size={12} color="#F8475F" fill="#F8475F" />
                        }
                    ...
                    {product.listPrice ? (
                      <h5 className="card-content-value">
                        De R${monetize(product.listPrice)}
                      </h5>
                    ) : (
                      <h5 className="card-content-value">
                        <br />
                    ...
                    <h3 className="card-content-value">
                      Por R${monetize(product.price)}
                    </h3>

                    {product.installments.length > 0 ? (
                      <p className="card-content-instalments">
                        ou em {product.installments[0].quantity}x de R$
                        {monetize(product.installments[0].value)}
                      </p>
                    ) : (
                      <p className="card-content-instalments">
                        <br />
